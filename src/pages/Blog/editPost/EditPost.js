...
export default function EditPost() {
	...
	useEffect(() => {
		fetch(`http://server-portfolioalex.vercel.app/post/${id}`)
			.then(response => {
	...
	async function updatePost(e) {
		...
		const response = await fetch('http://server-portfolioalex.vercel.app/blog/post', {
			method: 'PUT',
			body: data,
			credentials: 'include',
		});
	...
	async function deletePost(e) {
		...
		const data = new FormData();
		data.set('id', id);
		const response = await fetch(`http://server-portfolioalex.vercel.app/blog/edit/${id}`, {
			method: 'DELETE',
			body: data,
			credentials: 'include',
		});
